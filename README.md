...
5. Try to improve the model. Several options appear here:
    ...
    4. Take into account more / fewer words: in `main.py`, we load only the top 100k words in the embedding dictionary. You can load fewer or more; note that if you increase it a lot it will take a while to load and the generated `model.pth` will be very heavy.
    5. Change the model altogether: you could go for a recurrent neural network such as an [LSTM](http://pytorch.org/docs/master/nn.html#lstm)
